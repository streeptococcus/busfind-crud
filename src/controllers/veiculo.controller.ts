...
export async function getVeiculosByLinha(req: Request, res: Response): Promise<Response> {
    ...
    const conn = await connect();
    const veiculos = await conn.query("SELECT id_veiculo, nome_veiculo, modelo_veiculo veiculos.created_at, veiculos.modified_at, id_linha_fk FROM veiculos INNER JOIN linhas WHERE id_linha = ?", [id]);
    return res.json(veiculos[0]);
}